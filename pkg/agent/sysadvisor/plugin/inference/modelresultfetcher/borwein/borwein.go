...
func (bmrf *BorweinModelResultFetcher) initInferenceSvcClientConn() (bool, error) {
	...
	if len(bmrf.modelNameToInferenceSvcSockAbsPath) > 0 {
		modelNameToConn :=
			make(map[string]*grpc.ClientConn, len(bmrf.modelNameToInferenceSvcSockAbsPath))
		...
		if !allSuccess {
			...
			bmrf.modelNameToInferenceSvcClient =
				make(map[string]borweininfsvc.InferenceServiceClient, len(modelNameToConn))
			for modelName, conn := range modelNameToConn {
